...
//            ++dead_count;
	...
	game_state->my_hive_count = my_hive_count;
	game_state->enemy_hive_count = enemy_hive_count;
	...
	if (game_state->my_hives != 0)
		free(game_state->my_hives);
	if (game_state->enemy_hives != 0)
		free(game_state->enemy_hives);
	...
	if (my_hive_count > 0)
		game_state->my_hives = malloc(my_hive_count*sizeof(struct hive));
	else
		game_state->my_hives = 0;

	if (enemy_hive_count > 0)
		game_state->enemy_hives = malloc(enemy_hive_count*sizeof(struct hive));
	else
		game_state->enemy_hives = 0;
...
//            }
			...
			{
				--my_hive_count;
				game_state->my_hives[my_hive_count].row = i;
                game_state->my_hives[my_hive_count].col = j;
			}
			else if (current > 75 && current < 91)
			{
                --enemy_hive_count;
				
                game_state->enemy_hives[enemy_hive_count].row = i;
                game_state->enemy_hives[enemy_hive_count].col = j;
				game_state->enemy_hives[enemy_hive_count].player = current;
			}
            else 
			{
                --enemy_count;
    ...
    while (*data != 0) 
	{
        ...
        switch (*data) 
		{
            ...
            case 'a':
                ...
                break;
            case 'd':
                //game_info->map[offset] = var3 + 27; // do nothing with dead ants for now
                break;
            case 'f':
                game_info->map[offset] = '*';
                break;
			case 'h':
				game_info->map[offset] = var3 + 27; // put the hives where the dead ants used to be
				break;
        }
